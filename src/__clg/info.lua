...
local function _exec_clg_cli(...)
    ...
    local _proc = proc.spawn('bin/collegicoin-cli', _arg, {stdio = {stdout = 'pipe', stderr = 'pipe'}, wait = true})
...
local function _get_clg_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.clgd == 'running' then
    if am.app.get_configuration("NODE_PRIVKEY") then
        local _exitcode, _stdout, _stderr = _exec_clg_cli("-datadir=data", "masternode", "status")
        local _success, _output = _get_clg_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_clg_cli('-datadir=data', 'getblockchaininfo')
    local _success, _output = _get_clg_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_clg_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_clg_cli_result(_exitcode, _stdout, _stderr)
